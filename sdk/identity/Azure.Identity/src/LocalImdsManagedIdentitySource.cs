...
{
    internal class LocalImdsManagedIdentitySource : IManagedIdentitySource
    {
        ...
        private static readonly Uri s_imdsEndpoint = new Uri("http://localhost:40342/metadata/identity/oauth2/token");
        private static readonly IPAddress s_imdsHostIp = IPAddress.Parse("127.0.0.1");
        private const int s_imdsPort = 40342;
        private const int ImdsAvailableTimeoutMs = 10000;
        private const string ImdsApiVersion = "2020-06-01";
        ...
        public static async ValueTask<IManagedIdentitySource> TryCreateAsync(HttpPipeline pipeline, bool async, CancellationToken cancellationToken)
        {
            ...
            return available ? new LocalImdsManagedIdentitySource(pipeline) : default;
        }

        internal LocalImdsManagedIdentitySource(HttpPipeline pipeline)
        {
            _pipeline = pipeline;
        }
        ...
        public Request CreateRequest(string[] scopes)
        {
            ...
            string authorizationHeader = PreflightRequest(scopes);
        ...
        private string PreflightRequest(string[] scopes)
        {
            string resource = ScopeUtilities.ScopesToResource(scopes);
            ...
            if (response.Status == 401 && response.Headers.TryGetValue("WWW-Authenticate", out var wwwAuthenticationHeaderValue))
            {
                // Basic realm=C:\ProgramData\AzureConnectedMachineAgent\Tokens\2de39967-b092-4b76-b301-9dedf10cd19f.key
                if (!System.Net.Http.Headers.AuthenticationHeaderValue.TryParse(wwwAuthenticationHeaderValue, out var realmFilePath))
                {
                    return string.Empty;
                }
                var authorizationFilePath = realmFilePath.Parameter.Split('=')[1];
                if (System.IO.File.Exists(authorizationFilePath))
                {
                    try
                    {
                        var headerValue = System.IO.File.ReadAllText(authorizationFilePath);
                        return headerValue;
                    }
