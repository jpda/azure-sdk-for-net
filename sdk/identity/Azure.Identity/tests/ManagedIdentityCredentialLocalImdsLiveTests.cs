...
namespace Azure.Identity.Tests
{
    ...
    public class ManagedIdentityCredentialLocalImdsLiveTests : IdentityRecordedTestBase
    {
        public ManagedIdentityCredentialLocalImdsLiveTests(bool isAsync) : base(isAsync)
        {
        }

        [NonParallelizable]
        [Test]
        public async Task ValidateLocalImdsSystemAssignedIdentity()
        {
        ...
        [NonParallelizable]
        [Test]
        public async Task ValidateLocalImdsUserAssignedIdentity()
        {
            //if (string.IsNullOrEmpty(TestEnvironment.IMDSEnable))
            //{
            //    Assert.Ignore();
        ...
        private ManagedIdentityCredential CreateManagedIdentityCredential(string clientId = null, TokenCredentialOptions options = null)
        {
            ...
            //    ? new MockManagedIdentityClient(pipeline, clientId) { ManagedIdentitySourceFactory = () => new LocalImdsManagedIdentitySource(pipeline.HttpPipeline) }
            //    : new ManagedIdentityClient(pipeline, clientId);

            var client = new ManagedIdentityClient(pipeline, clientId);
